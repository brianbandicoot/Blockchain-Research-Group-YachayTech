...
int main(void){
  ...
  char newTx1[60];
  generateTx(&tx1,Alice,Bob,20); //Change the ammount of money to be send
  ...
  readBlockchain(&previousBlock,1); //Change this value if it is not the second block you are adding
  ...
  puts("\nStarting PoW-----------------------------------------------------------------\n");
  PoW(&B1,2); //Change difficulty if blocks where created in less than 10 minutes
  printBlock(&B1);
